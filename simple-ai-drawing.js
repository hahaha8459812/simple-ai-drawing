...
if (!seal.ext.find('simple-ai-drawing')) {
  ...
  seal.ext.registerStringConfig(ext, 'backendUrl', 'http://localhost:35814/process-image', '图生图后端服务地址（留空则禁用图生图）');
  seal.ext.registerTemplateConfig(ext, 'presetPrompts', [
    '手办化:Your task is to create a photorealistic, masterpiece-quality image of a 1/7 scale commercialized model or figurine based on the user\'s provided subject. The final image must be in a realistic style and environment. **Analysis First:** Before generating, analyze the subject. If the subject is a character (human, humanoid, or anthropomorphic), follow the "Character Instructions". If the subject is an object (e.g., vehicle, weapon, building), follow the "Object Instructions". **Character Instructions:** The model\'s face and likeness are the most critical elements. It must be a perfect, high-fidelity 3D translation of the character from the source image. The sculpt must be sharp, clean, and intricately detailed, accurately capturing the original artwork\'s facial structure, eye style, expression, and hair. The final result must be immediately recognizable as the same character. Do NOT generate a generic or abstract face. **Object Instructions:** The model\'s form and details are the most critical elements. It must be a perfect, high-fidelity 3D translation of the object from the source image. The model must be sharp, clean, and intricately detailed, accurately capturing all specific features, markings, and the overall design language of the original object. **Shared Scene Composition (Strictly follow these details for both characters and objects):** 1. **Model & Base:** Place the model on a computer desk. It must stand on a simple, circular, transparent acrylic base WITHOUT any text or markings. 2. **Computer Monitor:** In the background, a computer monitor must display 3D modeling software (like ZBrush or Blender) with the digital sculpt of the very same model visible on the screen. 3. **Artwork Display:** Next to the computer screen, include a transparent acrylic board with a wooden base. This board holds a print of the original 2D artwork that the model is based on. 4. **Environment:** The overall setting is a desk, with elements like a keyboard to enhance realism. The lighting should be natural and well-lit, as if in a room.'
